...
export default function OverviewView({
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        <h1 className="text-2xl font-black text-charcoal">Обзор сети</h1>
        <p className="text-muted text-sm mt-0.5">Главные показатели и точки, требующие внимания прямо сейчас</p>
      </div>

      {/* KPI strip — на тёмной плашке для контраста */}
      <div className="bg-charcoal2 rounded-2xl p-5">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="card p-5">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="text-sm font-bold text-charcoal">Точки с наибольшей недостачей</h3>
              <p className="text-xs text-muted mt-0.5">Куда смотреть в первую очередь</p>
            </div>
            <button onClick={() => onNavigate('locations')} className="text-xs text-orange-dark hover:underline flex items-center gap-1">
          ...
          <div className="flex flex-col gap-2">
            {topRed.map((l) => (
              <button
                ...
                className="flex items-center gap-3 p-3 rounded-xl text-left transition-colors hover:bg-surface"
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="font-bold text-charcoal text-sm">{l.locationName.replace('Bahandi ', '')}</div>
                  <div className="text-xs text-muted">{l.district}</div>
                </div>
                <div className="text-right">
                  <div className="text-base font-black" style={{ color: varianceColor(l.variancePercent) }}>{l.variancePercent}%</div>
                  <div className="text-xs text-muted">{l.unexplainedDeficit > 0 ? fmtMoney(l.unexplainedDeficit) : '—'}</div>
                </div>
        ...
        <div className="card p-5">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="text-sm font-bold text-charcoal">Топ подозрительных сотрудников</h3>
              <p className="text-xs text-muted mt-0.5">Аномальный уровень брака</p>
            </div>
            <button onClick={() => onNavigate('employees')} className="text-xs text-orange-dark hover:underline flex items-center gap-1">
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5">
                    <span className="font-bold text-charcoal text-sm">{emp.employeeName}</span>
                    {emp.medianMultiplier > 2 && <span className="badge badge-red text-[10px] px-1.5 py-0">×{emp.medianMultiplier}</span>}
                  </div>
                  <div className="text-xs text-muted">{emp.locationName.replace('Bahandi ', '')} · {fmtMoney(emp.totalWriteOffs)}</div>
                </div>
