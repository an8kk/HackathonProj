...
export default function ReconciliationView({ onOpenLocation }: { onOpenLocation: (id: string) => void }) {
  ...
  return (
    <div className="flex flex-col gap-5">
      <div>
        <h1 className="text-2xl font-black text-text-primary">Сверка с реальностью</h1>
      ...
      <div className="card p-5 bg-offwhite/50">
        <div className="flex items-center gap-2 mb-3">
          <Info className="w-4 h-4 text-amber-dark" />
          <h3 className="text-sm font-bold text-text-primary">Как считается необъяснённая недостача</h3>
        </div>
        <div className="text-sm text-text-primary leading-relaxed font-mono bg-white rounded-xl p-4 border border-stone-100 overflow-x-auto">
          <div>остаток на начало + приёмка − продажи (по техкартам iiko) − одобренные списания</div>
          <div className="text-text-muted">= сколько <b className="text-text-primary">должно</b> остаться на складе</div>
          <div className="mt-2">должно остаться − <b className="text-theft">факт по инвентаризации</b></div>
          <div className="text-theft font-bold">= необъяснённая недостача = хищение</div>
        </div>
