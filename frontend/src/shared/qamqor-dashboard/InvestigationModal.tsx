      ...
      >
        <div className="flex items-start justify-between mb-1">
          <div className="flex items-center gap-2.5">
            <div className="w-10 h-10 rounded-xl bg-red-light flex items-center justify-center flex-shrink-0">
              <ShieldAlert className="w-5 h-5 text-red" />
            </div>
            <div>
              <h3 className="font-black text-charcoal text-lg leading-tight">Открыть расследование</h3>
              <p className="text-xs text-muted">{location.locationName}</p>
            </div>
          </div>
        ...
        <div className="mt-3 mb-4 p-3 rounded-xl bg-red-light flex items-center justify-between">
          <span className="text-xs text-red font-medium">Необъяснённая недостача на момент открытия</span>
          <span className="text-sm font-black text-red">{fmtMoneyFull(location.unexplainedDeficit)}</span>
        </div>

        <label className="block text-xs font-semibold text-muted mb-1.5">Ответственный</label>
        <select className="select-input mb-3" value={assignee} onChange={(e) => setAssignee(e.target.value)}>
          {ASSIGNEES.map((a) => <option key={a} value={a}>{a}</option>)}
        </select>

        <label className="block text-xs font-semibold text-muted mb-1.5">Основание</label>
        ...
        <label className="block text-xs font-semibold text-muted mb-1.5">Заметка (необязательно)</label>
        <textarea
